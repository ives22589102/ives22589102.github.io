...
window.addEventListener("load", function() {
  // 遍歷所有class為content的元素
var contentElements = document.querySelectorAll('.content');

// 遍歷所有class為modal-content的元素
var modalContentElements = document.querySelectorAll('.modal-content');

// 遍歷所有class為eventcontent的元素
var eventContentElements = document.querySelectorAll('.eventcontent');

// 遍歷所有class為event_modal_content的元素
var eventModalContentElements = document.querySelectorAll('.event_modal_content');

// 把event-i元素添加到對應的event_modal_content-i元素中
for (var i = 0; i < eventContentElements.length; i++) {
  var eventContentElement = eventContentElements[i];
  var eventId = eventContentElement.getAttribute('id');
  var modalId = 'event_modal_content-' + eventId.split('-')[1];
  var modalContentElement = document.getElementById(modalId);
  if (modalContentElement) {
    modalContentElement.appendChild(eventContentElement);
  }
}

// 把content-i元素添加到對應的modal-content-i元素中
for (var i = 0; i < contentElements.length; i++) {
  var contentElement = contentElements[i];
  var contentId = contentElement.getAttribute('id');
  var modalId = 'modal-content-' + contentId.split('-')[1];
  var modalContentElement = document.getElementById(modalId);
  if (modalContentElement) {
